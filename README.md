...
- **`noreferrer` and `noopener`**: When developers designate links as noreferrer or noopener it can limit our access to the values used to populate frame origin and top-level site (for navigations without a valid previous primary main frame), respectively. As mentioned above, when window.opener is null or a link is marked noopener, we fall back on the live original opener chain. When calculating the frame origin in any siutation, we prefer the initiator origin, which is not affected by noreferrer. However, in the case that initiator origin is null (which is rare), we fall back on referrer. In which case, a link being marked noreferrer would leave us unable to construct a valid triple key.
